...
//// In the case of these label elements, we can treat our Field styling as the HTML5 default, removing the need for component-specific styling.
...
<Box as="p" className={styles.hint} id={`${id}-hint`} children={children} />

// Error

<Box as="p" className={styles.error} id={`${id}-error`}>{children} />
