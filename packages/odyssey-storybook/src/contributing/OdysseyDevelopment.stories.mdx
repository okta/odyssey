...
The repo is managed via [lerna](https://github.com/lerna/lerna) and [yarn workspaces](https://classic.yarnpkg.com/lang/en/docs/workspaces/)
...
To begin developing locally against Odyssey first clone the repository
and follow the subsequent steps:
...
title for example denote work that adds a new feature. If you are iterating
within your feature branch prior to merge use commit titles from the perspective
of a downstream developer reviewing a CHANGELOG in a future release. For example,
if you need to make changes based on code review feedback, do not title
your commit `fix: respond to PR feedback` instead squash changes together
using an interactive rebase or use a `chore: respond to PR feedback` title
which will not appear within CHANGELOG output.
