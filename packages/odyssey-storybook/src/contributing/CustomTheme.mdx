...
The `BackgroundProvider` component manages the contrast mode (`lowContrast` or `highContrast`) to detect the white and gray background and provides background context for child components. It is designed to help ensure that components are styled correctly based on the background they are rendered on.
...
By default, the `BackgroundProvider` will automatically detect the parent background color and determine the appropriate contrast mode. This is particularly useful when components need to adapt dynamically to different backgrounds for visual contrast consistency.
