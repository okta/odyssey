...
You can provide a custom "no response" state using the custom `<EmptyDataState>` component:

```tsx
noResultsPlaceholder={<EmptyDataState
...
<Canvas>
  <Story of={DataTableStories.CustomFilters} />
</Canvas>
...
<Canvas>
  <Story of={DataTableStories.FilterWithCustomRender} />
</Canvas>

You can use an out-of-the-box filter variant with a custom filter:

<Canvas>
  <Story of={DataTableStories.CustomFilterWithDefaultVariant} />
</Canvas>
