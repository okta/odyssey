...
To sandbox your application's styles, you need to use a Web Component which also includes a Shadow DOM. Odyssey natively supports the use of a Shadow DOM and even exports a helper function.

To create a Shadow DOM, simply use the `renderReactInWebComponent` in your app's top-level file (typically `index.ts`):

```tsx
import { renderReactInWebComponent } from "@okta/odyssey-react-mui/unified-ui";
