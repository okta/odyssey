// Fork of Hexo's toc.js helper
// Filters out any heading with a class on it to avoid TOCing examples
...
hexo.extend.helper.register('tocf', function(str, options = {}) {
  if (!cheerio) cheerio = require('cheerio');

  const $ = cheerio.load(str);
  const headingsMaxDepth = options.hasOwnProperty('max_depth') ? options.max_depth : 6;
  const headingsSelector = ['h1:not([class])', 'h2:not([class])', 'h3:not([class])', 'h4:not([class])', 'h5:not([class])', 'h6:not([class])'].slice(0, headingsMaxDepth).join(',');
