...
<aside class="ods-infobox is-ods-infobox-info" role="status">
  ...
  <h1 class="ods-infobox--title">Note: scope of work</h1>
  <section class="ods-infobox--content">
    <p>The assumed scope of work for this change includes updating the existing React components to utilize the new HTML structure, but not a reimplementation to use the components listed below.</p>
  </section>
</aside>
...
```html
<label class="ods-field--label">Destination</label>
...
Helper text for the field. The field hint is similar to the disallowed `placeholder` attribute. It should provide any necessary help text for the field, but is entirely optional. Appropriate examples include expected formatting or value types.

```html
<p class="ods-field--hint">Expected format: YYYY-MM-DD</p>
```
...
Text representing the validation state of the field. Today, we only offer styles for error messages, but success messaging is also possible.

```html
<p class="ods-field--error">This field cannot be left blank.</p>
```
...
As noted above, the Input is the actual UI control for the Field. Field should be able to accommodate any Input type that Odyssey currently offers.

</Description>

<Visual>
  <input class="ods-text-input" type="text" name="input-text-2" id="input-text-2" spellcheck="false" value="">
</Visual>
...
Together, an assembled Field would resemble the following structure:

```html
<!-- Pseudo-React -->
<Field optional>
  <Field.Label>First name</Field.Label>
  <Field.Hint>Please enter your first name.</Field.Hint>
  <Input/>
  <Field.Validation type="error">Numbers are disallowed.</Field.Validation>
</Field>
...
<div class="ods-field">
  ...
  <p class="ods-field--hint" id="full-field-hint">
    Please enter your first name.
  </p>
  <input class="ods-text-input" type="text" name="full-field" id="full-field" spellcheck="false" value="" autocomplete="fname" aria-describedby="full-field-hint full-field-error">
  <p class="ods-field--error" id="full-field-error"><span class="u-visually-hidden">Error:</span> Numbers are disallowed.</p>
</div>
```

Please see below for variant HTML options for Fields containing grouped Inputs.
...
In HTML, most of these states are assigned or applicable to the `<input>` element. However, those Input states are better reflected at the Field level. Some states may impact components other than Input.
...
- `:focus-within` When a child has `focus`. Should be the same as `:focus` unless input-specific
- `:hover` - When the Input is hovered by a pointing device.
- `:invalid` - When an Input has invalid contents/values.
- `:user-invalid` - When an Input is invalid, but only after a user has interacted with it. Identical to invalid.
- `:out-of-range` - When an Input is outside of range constrictions. Unnec until shipping range inputs, would mimic invalid.
- `:optional` - When an Input is optional. "Optional" label becomes present/visible.
...
- `:autofill` - When an Input has been auto-filled by the browser. Browsers and plugins provide bespoke styling to indicate when a field has been auto-filled. To preserve consistency of user-experience, it's best to leave these unstyled.
...
- `placeholder` - In most cases, we want to disallow the use of this attribute.
