...
import * as Tokens from "@okta/odyssey-design-tokens"; // TEMP. Remove when we update to `odysseyTokens`.
import {
  ArrowLeftIcon,
  ArrowRightIcon,
  CalendarIcon,
...
import { createOdysseyMuiTheme, DesignTokens } from "../theme";

const popupSpacingValue = 5;

export const createDatePickerTheme = (odysseyTokens: DesignTokens) => {
  odysseyTokens; // TEMP. Remove when we update to `odysseyTokens`.
  ...
  const datePickerTheme: ThemeOptions = {
    components: {
      MuiCalendarPicker: {
        styleOverrides: {
          root: ({ theme }) => ({
