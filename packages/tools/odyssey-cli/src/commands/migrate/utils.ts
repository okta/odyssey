...
export const formatMigrationLabel = (key: string): string => {
  ...
  return `${pc.cyan(source.component)} → ${pc.green(target.component)} ${pc.dim(`(${source.package} → ${target.package})`)}`;
};
...
/**
 * Creates a logger function that emits colorized messages to the console if not in CI mode
 */
export const createLogger = (isCI: boolean): Logger => {
  return ({ message, type }: EmitLog) => {
    const colored = logColor[type](message);
    if (isCI) {
      console[type](`[migrateComponent] ${message}`);
    } else {
      log[type](colored);
    }
  };
