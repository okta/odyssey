  ...
  return translationFiles
    .map<TranslationData | null>((file) => {
      ...
      const absoluteImportPath = resolve(
        ...CONFIG.TRANSLATION_TYPESCRIPT_PATH_SEGMENTS,
        jsFileName,
      );
      const fileName = file;
...
const prepareFileGeneration = ({ filename }: { filename: string }) => {
  log.processStart(`Generating ${filename}...`);
...
const writeFileWithHeader = async ({
  ...
  log.success(`Successfully generated ${filePath}`);
};
...
const generateI18nFiles = async ({
  ...
  const packageName = propertiesFileName ?? getPackageName();

  const translationFilesDirectory = resolve(
    ...CONFIG.TRANSLATION_TYPESCRIPT_PATH_SEGMENTS,
  );

  const hasTranslationFilesDirectory = await getHasFileOrDirectory(
    translationFilesDirectory,
  );
  if (!hasTranslationFilesDirectory) {
    log.warn(
...
export const generateI18nCommand: CommandModule<object, GenerateI18nArgs> = {
  builder: (yargs) =>
    yargs
      .option("defaultLanguageCode", {
        describe: "The language code to use as the default/fallback",
        type: "string",
        default: "en",
      })
      .option("propertiesFileName", {
        describe: "The name of the .properties file",
        type: "string",
        defaultDescription: "Than name of the package",
      }),
  command: "generate",
  describe: "Generates i18n resource and type files for the current package.",
