...
import * as DataViewStories from "./DataView [labs].stories";
...
Provide custom empty states for when there's no data or no search results. The `DataView` component provides two distinct states for handling these scenarios:

- **`isEmpty`**: Represents a state where no data exists at all. This is often seen in onboarding scenarios, such as when a user creates a new organization and no resources are available yet. For example:

  > "You don't have any widgets! Create one?"

  Use this state to guide users toward actions that populate data.

- **`isNoResults`**: Indicates that data exists but no matches were found for the current query or filters. For instance, if a user searches for "asdf" and no results are returned, the message might be:

  > "No data found for that query."

  This state helps users understand that their search or filter criteria need adjustment.

If `emptyPlaceholder` is not defined, `noResultsPlaceholder` will act as a fallback for the `isEmpty` state instead.
