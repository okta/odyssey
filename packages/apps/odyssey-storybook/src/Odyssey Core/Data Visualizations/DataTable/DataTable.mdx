...
> ⚠️ **Deprecated:** DataTable is deprecated. Use **DataView** for all new work.

## Migrating to DataView
...
- **getData** / **fetchDataFn** → `getData` (same args and return shape)
...
- **hasFilters**, **hasSearch**, **hasSearchSubmitButton** → same prop names on DataView
- **hasPagination**, **paginationType**, **currentPage**, **resultsPerPage**, **totalRows** → same prop names on DataView
- **hasRowSelection**, **hasRowReordering**, **onReorderRows** → same prop names on DataView
- **emptyPlaceholder**, **noResultsPlaceholder** → same props; optionally set `isEmpty` and `isNoResults` to control which state shows
- **searchDelayTime** → same prop name on DataView

### Keep a table-only layout

Lock layouts to keep the table experience while you migrate:
...
- Both DataTable and DataView support `emptyPlaceholder` and `noResultsPlaceholder`.
- DataView adds optional control flags:
  - `isEmpty`: force the empty state (e.g., onboarding) instead of relying on automatic detection.
  - `isNoResults`: force the “no results” state when data exists but the current query is empty.

### Migration checklist

1. Swap the import to DataView from `@okta/odyssey-react-mui/labs`.
2. Move column/table props into `tableLayoutOptions`.
3. Keep your existing `getData`, pagination, search, filter, and row selection hooks; rename `onChangeRowSelection` to `onRowSelectionChange`.
4. Lock `availableLayouts` to `["table"]` if you do not want list/grid yet.
5. Adopt `emptyPlaceholder` / `noResultsPlaceholder` and `errorMessage` as needed.
