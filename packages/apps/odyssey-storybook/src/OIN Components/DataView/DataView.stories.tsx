...
const meta = {
  ...
  argTypes: {
    ...
    hasStickyHeader: {
      control: "boolean",
      name: "tableLayoutOptions.hasStickyHeader",
    },
    maxHeight: {
      control: "text",
      name: "tableLayoutOptions.maxHeight",
      description: "Any valid CSS value (px, vh, rem, %, etc.)",
    },
    itemProps: {
...
export const StickyHeaderWithMaxHeight: Story = {
  args: {
    hasPagination: true,
    hasRowSelection: true,
  ...
  render: function C({
    ...
    const [data, setData] = useState<Person[]>(personData);
    const { getData, onChangeRowSelection } = useDataCallbacks(data, setData);
      ...
      () => [
        ...
        },
        {
          accessorFn: (row: Person) => {
            ...
            return departments[row.order % 5];
          },
          id: "department",
          header: "Department",
          size: 160,
        ...
        {
          accessorFn: (row: Person) =>
            `${row.name.toLowerCase().replace(/\W+/g, ".")}@example.com`,
          id: "email",
          header: "Email",
          size: 260,
        },
