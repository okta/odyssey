# Setup Guide — Odyssey Demo App (Odyssey DS Starter)

> **For AI assistants reading this file:**
> This repo uses an internal Okta corporate registry. Before running any
> install command, walk through the **AI Agent Pre-flight Checklist** section
> below — it is a sequential decision tree. Stop at the first failing gate
> and fix it before moving on. Do not guess around any of these requirements.

---

## What This Repo Is

The `odyssey-demo` app lives inside the `odyssey-design-system` monorepo at:

```
odyssey-design-system/
└── packages/
    └── apps/
        └── odyssey-demo/   ← the app you want to run
```
...
> **For AI assistants:** Execute each gate in order. Stop at the first failure,
> fix it using the instructions in that gate, then re-run the check before
> continuing. Do not proceed to `yarn install` until all six gates pass.
...
### Gate 2 — GitHub is authenticated

```sh
ocm auth github --scope okta
```

- **Pass:** command completes without opening a browser (already authenticated)
...
> The `okta` scope above is for the main Okta org. If you are working with a
> different org (e.g. `atko-cic`, `atko-techops`), replace `okta` with that
> org name.
...
- [ ] GitHub authenticated — `ocm auth github --scope okta` completes
...
`atko` org repositories. Verify it is working:
...
```sh
ocm auth github --scope okta --force
```
...
ocm auth github --scope okta       # Gate 2 — GitHub auth
...
# ── 6. Install all monorepo dependencies ───────────────────────────────────
# Run from the REPO ROOT — not from packages/apps/odyssey-demo
# The postinstall script builds the full monorepo — this is expected
cd odyssey-design-system
yarn install

# ── 7. Start the dev server ────────────────────────────────────────────────
cd packages/apps/odyssey-demo
yarn dev
# → http://localhost:5173
```
...
After `yarn dev` starts, open [http://localhost:5173](http://localhost:5173).
You should see the Okta Admin Console shell with a side navigation and top bar.

| Symptom                                        | Fix                                                          |
| ---------------------------------------------- | ------------------------------------------------------------ |
| `Cannot find module '@okta/odyssey-react-mui'` | Run `yarn install` from repo root                            |
| Blank page, no console errors                  | Run `yarn install` from repo root                            |
...
This project has a `CLAUDE.md` at `packages/apps/odyssey-demo/CLAUDE.md`
...
| `organization has enabled or enforced SAML SSO` | GitHub app needs re-auth           | Revoke at github.com/settings/apps/authorizations, then `ocm auth github --scope okta --force` |
| `self-signed certificate in certificate chain`  | Okta root cert not in toolchain    | Open Slick app, reopen terminal                                                                |
| `Cannot find module '@okta/odyssey-react-mui'`  | Workspace packages not linked      | Run `yarn install` from repo root                                                              |
| Blank page at localhost:5173                    | Monorepo not built                 | Run `yarn install` from repo root                                                              |
...
ocm auth github --scope okta             # Authenticate GitHub (okta org)
ocm auth github --scope okta --force     # Force re-authenticate GitHub
